...
- `VRC_MONITOR_WORLD_FETCH_COOLDOWN_MS`：**世界名回源失败冷却**（默认 `1800000`=30 分钟）。与 `WORLD_CACHE_TTL_DAYS`（新鲜度阈值）解耦：一次瞬时失败（网络抖动/限流/超时）后，该世界在冷却期内不再被后台重试，既压住「不可见世界每请求重试」的配额浪费，又保留快速自愈。非法值/0/负回落默认；正数小于 60s 时钳到 60s。
...
- **外部调用留痕规范**（PR 外部调用可观测性）：`[api]` = VRChat REST 调用，`[ext]` = 外部服务（PlanetVRC / X / BOOTH / Google Calendar / IMAP-OTP）。分级固定为「失败/超时/非 2xx → WARN；降级/兜底/缓存命中/跳过 → INFO；成功 → debug（>2000ms 升 INFO）」，且同一时间同步写入 ops_log（`kind='api'|'ext'`，`get_ops_log` 可查、保留 500 条）；聚合快照见 `GET /health` 的 `api` 字段（`api.client` / `api.ext` / `rateLimiter`）。新增外部调用点必须逐分支留痕，禁止静默降级。**限流等待例外（issue #192）**：`[limiter] 限流等待` 采用**聚合**而非逐条——串行批刷新（如逐好友 `await` 过限流器）会让行数 ≈ 批条数，故改为「去抖窗口」：窗口（默认 30s）内无新等待才输出一行 `限流等待聚合（近 30s 无新等待）：N 次，累计 Xms，单次最长 Yms，队列峰值 Z`；持续饱和时按最大跨度（默认 5min）定期输出（`持续饱和 ≥Xs`）；运行期与**优雅退出**（SIGINT/SIGTERM/`beforeExit` 均接线 `flushSlowWaitAgg()`）都不会静默，仅**无法执行清理**的退出会丢弃未满窗口的桶——即 SIGKILL / OOM-killer / `process.abort()` / 断电；普通 JS 崩溃（`uncaughtException`/`unhandledRejection` 亦接 `shutdown()`）不会丢桶。`/health` 的 `slowWaits`/`maxQueueLen` 仍是**逐次即时真值**（与聚合日志是同一事实的两种粒度），阈值 `SLOW_WAIT_MS=1000` 与 `/health` 字段语义都不得为降噪而改（改了等于静默重定义指标）。
- **X 博主世界推荐抓取通道（可选，PR #196 起）**：`x_scan_creators` / `x_world_digest` 需要 X 登录态才能稳定取时间线（Nitter 公共实例被反爬墙拦、匿名搜索接口 404）。以下变量均**可选**，未配置时该通道自动跳过、不影响其他功能：
  - `VRC_MONITOR_X_COOKIE` / `VRC_MONITOR_X_COOKIE_FILE`：X 登录 cookie（**账号级凭据**）——前者直接给 cookie 串，后者给文件路径；默认读 `<仓库>/data/x_cookie.txt`（`data/` 已被 .gitignore 排除）。**推荐用文件方式**：env 会让 cookie 出现在进程环境（`docker inspect` / `ps e` 可见）。**cookie 绝不入库、绝不进日志**（服务只把它放进请求头；错误文案只报 HTTP 状态）。cookie 数月会过期，过期时日志/报错会明确提示「cookie 失效」。
  - `VRC_MONITOR_X_USERTWEETS_QUERY_ID`（默认 `OeFjWKHutsuyWXZGmLr02A`）、`VRC_MONITOR_X_USERBYSCREENNAME_QUERY_ID`（默认 `KybxDj9RrADIITXlGG8kpw`）：X 站内 GraphQL 的 queryId 会随其前端 bundle 轮换，届时接口返回 404 并按文案提示更新此处。
  - `VRC_MONITOR_X_CREATOR_DELAY_MS`（默认 `1200`）：**博主之间的请求间隔**——X API 通道以用户本人账号发请求，串行无间隔会形成一轮 N 次突发（风控面）。
  - `VRC_MONITOR_X_RATE_LIMIT_BACKOFF_MS`（默认 `300000`=5min）：命中 429 后的**退避窗口**；窗口内 `x_scan_creators` 跳过后续博主并在结果里标 `skipped: 'x_rate_limited'`，不再持续打用户账号。
  - 其余通道开关见 `DEVELOPMENT.md` §3.4（Playwright 通道 `VRC_MONITOR_X_PLAYWRIGHT*` / Nitter 实例列表 / SearchTimeline `VRC_MONITOR_X_SEARCH_QUERY_ID` / t.co 解包 `VRC_MONITOR_X_RESOLVE_TCO` / 补抓阈值 `VRC_MONITOR_X_MIN_TWEETS`）。**注意 `VRC_MONITOR_X_SEARCH_QUERY_ID` 与本节两个 queryId 是同一失效模式**（X 轮换前端 bundle → 接口失效），但其运行时报错只有 HTTP 状态、不含变量名，排障时按本节与 §3.4 对照排查。
