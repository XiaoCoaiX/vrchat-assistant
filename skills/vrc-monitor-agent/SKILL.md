...
| `redeem_code` | **[redeem 插件] 提交兑换码**：`POST /reward/redeem`。码是**一次性消耗品**（成功不可回滚）。返回换到的物品/礼包（含 `itemType`/`contains`/`inventoryId`）；**含礼包时需再 `claim_bundle`** 才会真到手。失败如实返回 `{ok:false,status,error}`（码失效/已用/拼错） |
...
| `claim_bundle` | **[redeem 插件] 领取（打开）礼包**：`POST /inventory/{inventoryId}/consume` → 到手物品清单（`name`/`itemType`/`description`/`acquisition`）。领取后从待领列表消失，**不可重复领取** |
| `get_inventory_items` | **[redeem 插件] 库存物品**：`GET /inventory`（`type` 按 itemType 过滤如 nameplateEffect，`limit` ≤100）。完整库存视图，用于核对兑换/领取是否到账——核心 `get_inventory_global` 只给账号级全局物品 |
