...
> **当前架构（PR-2 插件化重构后）**：功能以「核心域工具 + 插件域工具」分层存在，统一由注册表（`core/registry.js`）按 `core/tool-order.json` 的**顺序混合索引**输出。插件通过 `core/plugin-loader.js` 加载、按 `core/plugin-api.js` 的 7 面 API 与核心交互，**不触碰全局 `ctx`**。旧的三件套（`mcp-definitions.js` / `rpc-router.js` / `core/handlers/*`）已被替换——本文档以此现状为准。
...
`buildPluginApi(pluginName, deps)` 构造**插件唯一合法的交互面**，共 7 面（契约 v1.3）：
...
| `plugin-api.js` | 插件 API 面。构造插件唯一合法的交互对象（7 面） | `buildPluginApi` |
...
start-monitor.js
  ...
  │     └── core/plugin-api.js(buildPluginApi → 7 面 API)
